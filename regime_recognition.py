...
plot_clusters(principalcomponents, cluster.KMeans, (), {'n_clusters':4})

kmean_cluster_labels = set(TS['clusters'])

#plot kmeans HELP
fig,ax=plt.subplots()
TS[['S&p Comp P']].plot(ax = ax, style ='-')

x = np.linspace(0, 1, 10)
number = len(kmean_cluster_labels)
top_clusters = number
number = top_clusters
...
for i in range(0,top_clusters):
    kmean_cluster_label = df_cf.iloc[i][0]
    if (kmean_cluster_label >= 0):
#        TS[['10 YR']][TS['clusters'] == cluster_label].plot(ax = ax, style =':', color = colors[cluster_label])
        TS[['S&p Comp P']][TS['clusters'] == cluster_label].plot(ax = ax, color = colors[cluster_label])
